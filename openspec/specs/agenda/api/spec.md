<!-- Source: openspec/changes/archive/agenda-http/specs/agenda/api/spec.md -- synced 2026-06-02 (agenda-http archive) -->
# Agenda API
...
#### Scenario: GET /api/me returns 200 with the current user
- **Given** any authenticated user
- **When** the client calls `GET /api/me`
- **Then** the response is `200` and the body is `{"data":{"id","name","email","role"}}`
