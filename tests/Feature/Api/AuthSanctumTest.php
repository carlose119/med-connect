 ...
 * integration on /api/me (REQ-API-1 + REQ-API-2 + REQ-API-5).
 *
...
it('honors the ?tz= query param on the /api/me route (200, no throw)', function (): void {
    $user = User::factory()->create();

    $this->actingAs($user, 'sanctum')
        ->getJson('/api/me?tz=America/New_York')
        ->assertStatus(200)
        ->assertJsonPath('data.id', $user->id);
});
